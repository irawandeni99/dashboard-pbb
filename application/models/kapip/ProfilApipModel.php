<?php
	class ProfilApipModel extends CI_Model{
	    ...
	    public function FilterProfil($cthn,$ckode)
	    {
			
			$cgroup = $this->session->userdata('is_admin');
	 
			$query = 'SELECT SUM(p1)p1,SUM(p2)p2,SUM(p3)p3,SUM(p4)p4,SUM(p5)p5,SUM(p6)p6,SUM(p7)p7 FROM(
						SELECT 
						CASE WHEN kd_profil=1 THEN jml END AS p1,
						CASE WHEN kd_profil=2 THEN jml END AS p2,
						CASE WHEN kd_profil=3 THEN jml END AS p3,
						CASE WHEN kd_profil=4 THEN jml END AS p4,
						CASE WHEN kd_profil=5 THEN jml END AS p5,
						CASE WHEN kd_profil=6 THEN jml END AS p6,
						CASE WHEN kd_profil=6 THEN jml END AS p7
						FROM(
						SELECT  a.kd_profil,a.nm_profil,(SELECT COUNT(*) FROM ms_group_menu_profil
						WHERE tahun="'.$cthn.'" AND kd_apip="'.$ckode.'" AND id_group="'.$cgroup.'" AND kd_profil=a.kd_profil)jml FROM ms_profil a
						)z
						)zz';
