<?php
	class PotensiModel extends CI_Model{
	    ...
	    public function getPotensiKecamatan()
	    {		 
			$query = "SELECT * FROM vpotensi_pajak_kecamatan ";
	    	$sql = $this->db2->query($query)->result();
	    	return $sql;
	    }

	    public function getPotensiKelurahan($kec)
	    {		 
			$query = "SELECT * FROM vpotensi_pajak_kelurahan where kd_kecamatan='".$kec."'";
	    	$sql = $this->db2->query($query)->result();
	    	return $sql;
	    }
		...
		public function getInstansi()
	    {
			...
			if($akses==3){
				$where="where kd_instansi='".$instansi."'";
			}else{
				$where='';
			}
		 
			$query = "SELECT * FROM ms_config ".$where ;
	    	$sql = $this->db->query($query)->result();
	    	return $sql;
	    }
