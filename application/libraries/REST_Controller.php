...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->input->is_ajax_request() === FALSE && $this->config->item('rest_ajax_only'))
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => 401,
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => 403,
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE)
        {
            ...
            {
                $response = [$this->config->item('rest_status_field_name') => 401, $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_time_limit')];
            ...
            if($authorized === FALSE)
            {
                ...
                $response = [$this->config->item('rest_status_field_name') => 401, $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_permissions')];
                $this->response($response, self::HTTP_UNAUTHORIZED);
        ...
        {
            $response = [$this->config->item('rest_status_field_name') => 401, $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_address_time_limit')];
            $this->response($response, self::HTTP_UNAUTHORIZED);
