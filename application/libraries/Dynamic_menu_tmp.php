...
class Dynamic_menu {
	...
	{
		$menu = array();
		$data 	= $this->ci->encrypt->decode($this->ci->session->userdata('mn'));		$dtMenu = array();				if ($data != ''){
		$res = $this->ci->db->query("SELECT a.*, 
										CASE WHEN parent_id > 0 THEN 
											(select title from dyn_menu b where b.id = a.parent_id ) 
											ELSE 0 END AS parentName 
										FROM `dyn_menu` a 
										WHERE id IN  ( ".$data.") ")->result_array();
		foreach ($res as $row) {
    ...
    function build_menu2 ($table = 'dyn_menu', $type = '0')
    {
        $menu = array();
