	...
	<div class="panel-body"
							...
							<div class="content-form" style="display:block;">
								...
								<div class="panel-footer">
									<center>
										<a href="<?= base_url($this->dynamic_menu->EncryptLink('kapip-master-penilaian-mandiri'));?>" class="btn btn-danger btn-lg"><i class="fa fa-arrow-circle-left"></i> KEMBALI</a>
...
$('#cblevel').select2({
 ...
 $(document).on("click", ".showTambah-level", function() {		  
   ...
   $(document).on("click", "#btntambah_penilaian", function(e) {  
		...
		document.getElementById("uraian_penilaian").focus();
	});
...
$(document).on("click", ".simpan-penilaian", function() {		
		...
		$.ajax({
					type: 'POST',
					data: ({ctahun,cinstansi,celemen,ctopik,clevel,curaian,cpenjelasan,cbukti,clangkah}),
					dataType: "json",
...
$(document).on("click", ".update-penilaian", function() {		
		...
		$.ajax({
					type: 'POST',
					data: ({ctahun,cinstansi,celemen,ctopik,clevel,cpenilaian,curaian,cpenjelasan,cbukti,clangkah}),
					dataType: "json",
