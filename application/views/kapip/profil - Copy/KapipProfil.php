	...
	<div class="panel-body">
      ...
      <div class="col-md-12">
				<div class="pull-right <?=$hidden; ?>">
					<a href="<?=base_url(EncryptLink('kapip-dataumum-profil-apip/add')); ?>" type="button" name="tambah" id="tambah" class="btn btn-primary btn-lg"><i class="fa fa-plus"></i> Tambah</a>
				</div>
      </div>
    </div>
    <div class="row">
...
/*       var noreg = '<?= $_GET["noreg"] ?>';
      var no_lhp = '<?= $_GET["no_lhp"] ?>';
      if (noreg != '' && no_lhp != '') {
        $.ajax({
          method: 'POST',
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", ".tlanjut", function() {
          ...
          var asp     = $(this).attr("data-aspek");
          var tem     = $(this).attr("data-temuan");
        	document.location.href = '<?php echo base_url('tlhp-bpk-lapkeu/tindak-lanjut'); ?>?noreg='+noreg+'&no_lhp='+no_lhp+'&asp='+asp+'&tem='+tem;
...
<script type="text/javascript">
	...
	$(document).on("click", ".showDetailApip", function() {
		var noreg 		= $(this).attr("data-kd");
		var tahun 		= $(this).attr("data-tahun");
        var nama 	= $(this).attr("data-nama");
        $.ajax({
	      method: 'POST',
	      url: '<?php echo base_url('tlhp-bpk-lapkeu/get-detail-lhp'); ?>?noreg='+noreg+'&no_lhp='+no_lhp,
	    })
	    .done(function(data) {
	    	$('#listDetail').html(data);
			$('#myModal').modal('show');
	...
	$(document).on("click", ".cetak-tlhp", function() {
		...
		swalWithBootstrapButtons.fire({
		  ...
		  if (result.value) {
		    ...
		    window.open('<?php echo base_url("tlhp-bpk-lapkeu/excel"); ?>?noreg='+noreg+'&lhp='+lhp, '_blank');	
		  } else if (
		    /* Read more about handling dismissals below */
		    result.dismiss === Swal.DismissReason.cancel
		  ) {
		  	
		        window.open('<?php echo base_url("tlhp-bpk-lapkeu/pdf"); ?>?noreg='+noreg+'&lhp='+lhp, '_blank');	
