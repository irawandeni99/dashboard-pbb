<?php
	...
	class ManualController extends CI_Controller {
		...
		public function __construct(){
			parent::__construct();
			$this->load->model('admin/Manual_model', 'm_model');
		}

		public function index(){
			$data['view'] = 'admin/manual/manualView';
		...
		public function get_combo_prov(){
			echo $this->m_model->getprov();	
		}

		public function get() {
			$list = $this->m_model->get_all_manual();
	        $data = array();
	        $no = $_POST['start'];
	        foreach ($list as $field) {
	            ...
	            $row[] = '<center>
	            	<a href="#" data-toggle="tooltip" title="preview" class="btn btn-default btn-flat btn-xs preview" data-id="'.$field->id.'" data-nama="'.$field->judul.'"><i class="lnr lnr-eye"></i></a>
		 	    	<a href="#" data-toggle="tooltip" title="Edit" class="btn btn-info btn-flat btn-xs edit hidden" data-id="'.$field->id.'" data-nama="'.$field->judul.'"><i class="lnr lnr-pencil"></i></a>
		 	    	<a href="#" data-toggle="tooltip" title="Hapus" class="dropdown-item btn btn-danger btn-flat btn-xs hidden tombol-hapus" data-id="'.$field->id.'" data-nama="'.$field->judul.'"><i class="lnr lnr-trash"></i></a></td>
		 	    	</center>';
	            $data[] = $row;
	        }
	        ...
	        $output = array(
	            "draw" => $_POST['draw'],
	            "recordsTotal" => $this->m_model->count_all_manual(),
	            "recordsFiltered" => $this->m_model->count_filtered_manual(),
	            "data" => $data,
	        );
		...
		public function add(){
			...
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$result = $this->m_model->add_daerah($data);
					if($result){
		...
		public function edit($id = 0){
			...
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$data = $this->security->xss_clean($data);
					$result = $this->m_model->edit_daerah($data, $id);
					if($result){
		...
		public function get_kab() {
	        ...
	        $output = array(
	            "draw" => $_POST['draw'],
	            "recordsTotal" => $this->m_model->count_all_kab(),
	            "recordsFiltered" => $this->m_model->count_filtered_kab(),
	            "data" => $data,
	        );
	        echo json_encode($output);
		}
		...
		public function getmax(){
			 ...
			 $max_id=$this->m_model->get_max_daerah($level,$header);
			 echo $max_id;
		}
